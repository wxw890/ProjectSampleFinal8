...
public class freeBoardManager {
	...
	public static List findKeyword(BoardDto dto){
		...
		list = session.selectList("findKeyword1", dto);
			...
			System.out.println("findKeyword 에러"+err);
		}
		finally {
			session.close();
		}
	...
	public static void write(BoardDto dto){
		...
		session.commit();//insert, update, delete는 commit()을 해줘야 완성이 된다.
			...
			System.out.println("BoardDto에러"+err);
		}
		finally {
			session.close();
		}
	...
	public static BoardDto finBySeq(int seq){
		...
		SqlSession session = sqlFactory.openSession();
		try{
		 dto = session.selectOne("findBySeq1", seq);
		}
		catch(Exception err){
	...
	public static void Reply(BoardDto dto){
		...
		session.commit();
			...
			System.out.println("Reply 에러"+err);
		}
		finally {
			session.close();
		}
	...
	public static void PosUpdate(BoardDto dto){
		...
		session.commit();
			...
			System.out.println("BoardDto 에러"+err);
		}
		finally {
			session.close();
		}
